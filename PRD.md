...
> **上次更新**: 2026/06/23 — 基于项目书全面对齐：补充产品定位原则、更新竞品分析、砍掉社交/数据同步/票务、更新开发状态表
...
**核心定位原则：**
- **个人追踪工具，不是公共信息聚合平台**：不做全城剧目数据库，不接入官方票务系统
- **不碰票务**：不卖票、不比价、不追踪折扣、不做二手转票
- **不碰公共排期信息**：不提供各城市演出展览查询，不做信息聚合

**信息中枢 vs 信息入口**：排期天菜是一个**信息中枢**——用户从各个渠道（微信群、公众号、小红书、朋友推荐）获取的信息，手动录入到 App 里，形成自己的私密、可靠、有结构的排期数据。信息源是多渠道、社交化的，排期天菜负责的是后续的追踪管理和决策支持。
...
| 维度 | 票夹Drama | 韭菜盒子 | 排期天菜 |
|------|----------|---------|---------|
| **定位** | 剧院观众记录工具 | 演出行业信息工具 | 个人排期管理工具 |
| **核心场景** | 看完后记录、晒票根 | 演出信息聚合 | 关注 → 排期 → 决策 → 记录 → 复盘 |
| **排期能力** | 无 | 有限 | 可视化排期流、演员卡司对比、全周期追踪 |
| **数据来源** | 手动录入 | 平台聚合 | 手动录入（信息中枢） |
| **视觉** | 白底+文字列表 | 标准工具 | 剧场暗黑风，沉浸感 |
| **核心价值** | 记录 | 信息 | **排期决策** |

**排期天菜的差异化核心**：不与售票平台竞争（大麦/猫眼），不与记录工具竞争（票夹Drama），不做信息聚合（韭菜盒子）。聚焦在"买之前的决策阶段"——这是现有工具完全没有覆盖的环节。
...
**定位**：OCR 是辅助录入手段，不是产品入口。用户的核心输入方式是手动录入，OCR 用于加速特定场景（如拍排期表批量导入）。
...
|------|------|------|
| 用户系统 | ✅ 完成 | 注册/登录/多用户隔离 |
| 日历视图 | ✅ 完成 | 月/双周/周视图、农历、年历、筛选 |
| 排期甘特图 | ✅ 完成 | 3日/7日双模式、海报卡片、灯光秀视觉 |
| 月度工作台 | ✅ 完成 | 海报网格画廊、月份筛选、排期流状态管理 |
| 剧目管理 | ✅ 完成 | 表格录入、场次级排期流、批量操作 |
| OCR 识别 | ✅ 完成 | 三引擎 + 排期/卡司解析 + 知识库纠错 |
| 个人中心 | ✅ 完成 | Hero指标、条形图、演员/剧场排名、金额统计、月度节奏图 |
| 场次详情页 | ✅ 完成 | 展示+编辑分离、票根管理、待办清单 |
| 数据备份恢复 | ✅ 完成 | JSON + 海报图片导出 |
| Web Demo 模式 | ✅ 完成 | 自动创建demo账号 + 种子数据 |
| GitHub Pages 部署 | ✅ 完成 | 自动部署工作流 |
| 设置页 | ✅ 完成 | 备份恢复、重置、退出登录 |
| 排期板灯光秀 | 🔄 打磨中 | 第一阶段视觉已落地，双指缩放/边缘侧光待第二阶段 |
| 演员追踪 | ⏳ 待规划 | 标记关注演员、出场日历 |
| 社交分享 | ❌ 已砍 | 个人工具，不做社交 |
| 数据同步 | ❌ 已砍 | 纯本地存储，数据主权归用户 |
| 公共排期聚合 | ❌ 已砍 | 不做信息入口 |
| 票务管理 | ❌ 已砍 | 不碰票务 |
...
以下功能经评估不在产品核心范围内，不再规划：
- ~~社交分享~~ — 个人工具，不做社交
- ~~数据同步（WebDAV/云端）~~ — 纯本地存储，数据主权归用户
- ~~公共排期信息聚合~~ — 不做信息入口
- ~~票务信息完善（购票渠道/订单号）~~ — 不碰票务
...
短期以体验优化为主，中期围绕"追踪-统计"增强用户粘性。技术栈选型务实，Flutter 跨平台覆盖目标场景，OCR + 知识库是核心差异化壁垒。

核心定位不变：**个人追踪工具，不是公共信息聚合平台**。不碰票务，不碰公共排期信息，聚焦在"买之前的决策阶段"——这是现有工具完全没有覆盖的环节。
